...
y_train=train.Label
x_train = train.drop(['Label', 'instance weight','migration code-change in msa','migration code-change in reg','migration code-move within reg','migration prev res in sunbelt'], axis=1)
...
# for item in x_train.values:
	...
	# for thing in item:
		# if(thing == " NA"):
			# x_train.drop(item, axis=1)
			# print type(item)
	# x_train = x_train.drop(x_train.item == " NA")

print x_train.size
# dt_classifier = DecisionTreeClassifier()
# dt_classifier.fit(x_train,y_train,sample_weight=train['instance weight'].values)
# tree.export_graphviz(dt_classifier, out_file='tree.dot')
